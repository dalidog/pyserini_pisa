...
class PisaIndexer(ps.LuceneIndexer):
  def __init__(self, path, text_field='text', mode=PisaIndexingMode.create, stemmer='porter2', threads=1, batch_size=100_000):
    ...
    #overwrite or create
    self.stemmer = pyserini_pisa.PisaStemmer(stemmer)
    self.threads = threads
    self.batch_size = batch_size
    # should we keep this batch_size because 
    # it determines how many documents are processed in each batch when building the PISA index.

  def index(self, it):
    path = Path(self.path)
    if pyserini_pisa.PisaIndex.built(self):
